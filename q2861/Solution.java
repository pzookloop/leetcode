...
//    }
    ...
    public int maxNumberOfAlloys(int n, int k, int budget, List<List<Integer>> composition, List<Integer> stock, List<Integer> cost) {
        ...
        for (List<Integer> comp : composition) {
            ...
            while (left <= right) {
                int mid = (left + right) >>> 1;
                if (isEnough(budget, mid, comp, stock, cost)) {
                    left = mid + 1;
    ...
    private boolean isEnough(long budget, int metalNum, List<Integer> curMachine, List<Integer> stock, List<Integer> cost) {
        for (int i = 0; i < curMachine.size(); i++) {
            long m = Math.max((long) metalNum * curMachine.get(i) - stock.get(i), 0);
            if (m * cost.get(i) > budget) {
                return false;
            }
            budget -= m * cost.get(i);
        }
